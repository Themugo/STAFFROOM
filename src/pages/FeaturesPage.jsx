...
import {
  useLanding, useFeatures, useCTA, useSettings, useNavigation,
    ...
    <section className="relative pt-32 lg:pt-44 pb-16 lg:pb-24 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-4xl mx-auto">
          ...
          >
            Powerful, modular tools that work together — so your team can focus on people, not paperwork.
          </p>
        </div>
      </div>
    </section>
...
function FeatureGrid({ features }) {
  const grouped = useMemo(() => groupBy(features, 'category'), [features])
    ...
    <section className="py-16 lg:py-24 bg-gray-50 dark:bg-slate-900/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {categories.map((category, ci) => (
          <div key={category} className={ci > 0 ? 'mt-20 lg:mt-28' : ''}>
            <div data-reveal className="mb-10 lg:mb-14">
              <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-brand-50 dark:bg-brand-500/10 border border-brand-200 dark:border-brand-500/20 text-xs font-semibold uppercase tracking-wider text-brand-700 dark:text-brand-300 mb-3">
                <Sparkles className="h-3.5 w-3.5" />
                {category}
              </span>
